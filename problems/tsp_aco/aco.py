...
class ACO():
    ...
    def __init__(self,
        ...
        self.distances = torch.tensor(distances, device=device) if not isinstance(distances,
                                                                                  torch.Tensor) else distances
        self.n_ants = n_ants
        self.decay = decay
        self.alpha = alpha
        self.beta = beta
        ...
        self.heuristic = torch.tensor(heuristic, device=device) if not isinstance(heuristic,
                                                                                  torch.Tensor) else heuristic
    ...
    def gen_path(self, require_prob=False):
        ...
        '''
        # start = torch.zeros((self.n_ants,), dtype=torch.int32, device=self.device)
